...
class RandomCrop(object):
    ...
    def __call__(self, data):
        ...
        c, w, h, d = data.shape
        ww, hh, dd = self.size
        output = torch.zeros((c, ww, hh, dd), dtype=torch.float32)
    
        if w>=ww:
            x = np.random.randint(0, w - ww + 1)
        else:
            x = 0
            ww = w
        if h>=hh:
            y = np.random.randint(0, h - hh + 1)
        else:
            y = 0
            hh = h
        if d >=dd:
            z = np.random.randint(0, d - dd + 1)
        else:
            z = 0
            dd = d
        
        output[:, :ww, :hh, :dd] = data[:, x:x+ww, y:y+hh, z:z+dd]
...
class NineCrop(object):
    def __init__(self, size, op=0):
        super(NineCrop, self).__init__()
        self.size = size  
        self.op = op  
...
class Inserter_seg(object):
    ...
    def __init__(self, size, rand=True):
        super(Inserter_seg, self).__init__()
        self.size = size
        self.rand = rand
    def __call__(self, input_data):
        ...
        output_label = np.zeros((1, ww, hh, dd)).astype(np.float32)
        if self.rand:
            ...
            try:
                ...
                output_label[:, x:x+w, y:y+h, z:z+d] = label
            except:                
                target_shape = (c, ww, hh, dd)
                output = resize(data, target_shape, anti_aliasing=True)
                output_label= resize(label, (1, ww, hh, dd), anti_aliasing=True)
        else:
            try:
                x = (ww - w)//2
                y = (hh - h)//2 
                ...
                target_shape = (1, ww, hh, dd)
                output = resize(data, target_shape, anti_aliasing=True)
                output_label= resize(label, (1, ww, hh, dd), anti_aliasing=True)                
        return {'image': output, 'label':output_label}
...
class Inserter_bk(object):
    ...
    '''
    def __init__(self, size, rand=True):
        super(Inserter_bk, self).__init__()
        self.size = size
        self.rand = rand
    def __call__(self, input_data):
        data = input_data['image']
        ...
        if self.rand:
            ...
            try:
                ...
                target_shape = (c, ww, hh, dd)
                output = resize(data, target_shape, anti_aliasing=True)                
        return {'image': output, 'label':label}

class DCE_Channel(object):
    '''
            Insert the data into a fixed size
            data is of dim: channel x width x height x depth
            return torch tensor
    ...
    '''
    def __init__(self):
        super(DCE_Channel, self).__init__()
...
class MultimodalInserter(object):
    ...
    def insert_dce(self, data1, size):
        c, w, h, d = data1.shape
        
        ww, hh, dd = size
        output1 = np.zeros((c, ww, hh, dd)).astype(np.float32)
        ...
        if self.rand:
            try:
                ...
                y = np.random.randint(0, hh - h + 1)
                z = np.random.randint(0, dd - d + 1)
                output1[:, x:x+w, y:y+h, z:z+d] = data1
               
            except:
                target_shape1 = (6, ww, hh, dd)
                
                output1 = resize(data1, target_shape1, anti_aliasing=True)
...
class MultimodalInserter_New(object):
    '''
        Insert the data into a fixed size.
        data is of dim: channel x width x height x depth.
        return torch tensor.
    ...
    '''
    def __init__(self, dce_size, dwi_size, t2_size, rand=True):
        super(MultimodalInserter_New, self).__init__()
        self.dce_size = dce_size
        self.dwi_size = dwi_size
        self.t2_size = t2_size
        self.rand = rand
...
class MultimodalResizer(object):
    ...
    def __call__(self, data):
        ...
        dce, dwi, t2, pre = data['dce'], data['dwi'], data['t2'], data['pre']
        ...
        t2_output = torch.from_numpy(t2_output)
        pre_output = self.resize(pre, self.dce_size)
        pre_output = torch.from_numpy(pre_output)
        return {'dce':dce_output, 'dwi':dwi_output,'t2':t2_output, 'pre':pre_output}
...
class SixFlipper(object):
    ...
    def __init__(self, op):
        '''
        op: Integer value that determines the flipping operation to be applied:
            0 -> Flip along width (axis=1)
            1 -> Flip along height (axis=2)
            2 -> Flip along depth (axis=3)
            3 -> Flip along width and height
            4 -> Flip along width and depth
            5 -> Flip along height and depth
        '''
        self.op = op
    def __call__(self, data):
